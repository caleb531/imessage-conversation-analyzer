<script lang="ts">
  ...
  let {
    selectedContact = $bindable<string | undefined>(),
  } = $props();
...
<article class="contact-picker">
    {#if contactsLoading}
                ...
                <Combobox.Viewport class="contact-picker__viewport">
                    ...
                    {#each filteredItems as item (item.value)}
                        ...
                        {#snippet children({ selected })}
                            ...
                            {#if selected}
                            <span class="contact-picker__check" aria-hidden="true">✓</span>
                            {/if}
                        {/snippet}
                        </Combobox.Item>
                    {/each}
        ...
        {#if !comboboxItems.length}
            <p class="contact-picker-status contact-picker-status--empty">No contacts are available.</p>
        {/if}
        {#if selectedContact}
